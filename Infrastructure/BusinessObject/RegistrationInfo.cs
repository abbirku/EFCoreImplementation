    ...
    public class RegistrationInfo
    {
        ...
        public ValidationModel IsValid()
        {
            if(StudentRegistration.StudentId == 0)
                return new ValidationModel { IsValid = false, Message = "Please select a student" };

            var student = _studentRepository.GetById(StudentRegistration.StudentId);
            if (student == null)
                return new ValidationModel { IsValid = false, Message = "Selected student does not exists" };

            if (StudentRegistration.CourseId == 0)
                return new ValidationModel { IsValid = false, Message = "Please select a course" };

            var course = _courseRepository.GetById(StudentRegistration.CourseId);
            if (course == null)
                return new ValidationModel { IsValid = false, Message = "Selected course does not exists" };
            ...
            if(result < 0 && StudentRegistration.EnrollDate.Year <= 2019) 
                return new ValidationModel { IsValid = false, Message = "Registration year must be after 2019" };

            return new ValidationModel { IsValid = true, Message = "Registration information is valid" };
        }
